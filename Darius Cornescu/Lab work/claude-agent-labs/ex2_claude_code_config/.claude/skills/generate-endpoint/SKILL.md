...
# task 4: run in an isolated, forked context so its work does not pollute the
# main conversation.
context: fork
# task 4: this skill may ONLY read and write files. Bash is absent from the
# allow-list, so any request to run a shell command is refused.
