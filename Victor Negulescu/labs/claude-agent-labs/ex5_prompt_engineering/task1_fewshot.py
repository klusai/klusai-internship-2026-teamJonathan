...
"Uploads over 2 MB silently fail — no error, nothing happens." -> bug
"Add keyboard shortcuts for power users." -> feature
"Is there an API rate limit, and what is it?" -> question
"Clicking Save twice creates duplicate records." -> bug
"Please support CSV export in addition to PDF." -> feature
"Do you offer a discount for non-profits?" -> question
"""
...
# --- Recorded accuracy (run once with FEWSHOT="", then again with FEWSHOT filled) ---
# Zero-shot accuracy  (non-debatable tickets, FEWSHOT empty): 17/17 = 100%  (model=claude-haiku-4-5)
# Few-shot accuracy   (non-debatable tickets, FEWSHOT filled): 17/17 = 100%  (model=claude-haiku-4-5)
# Haiku already achieves ceiling on this task; the debatable tickets (8, 13, 19) are where
# few-shot examples can anchor the model to a consistent stance on edge cases.
