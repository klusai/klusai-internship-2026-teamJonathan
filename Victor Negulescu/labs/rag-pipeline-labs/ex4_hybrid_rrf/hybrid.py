...
def reciprocal_rank_fusion(rankings: list[list[str]]) -> list[str]:
	...
	for ranking in rankings:
		for pos, doc_id in enumerate(ranking, 1):
			scores[doc_id] = scores.get(doc_id, 0.0) + 1.0 / (pos + 1)
...
# --- Recorded results (python hybrid.py) ---
# RRF truth table: all 3 cases PASS.
#
# recall@k over the 7 clean queries:
# method              @1    @2    @3
...
#
# Complementarity:
#   Q3 (Halibut — rare term): bm25@1=correct, semantic@1=wrong  → RRF keeps the BM25 win
#   Q4 (attrition paraphrase): bm25@1=wrong,  semantic@1=correct → RRF keeps the semantic win
#
# Why RRF merges by rank and not raw score:
#   BM25 and embedding similarity live on completely different scales — a BM25 score
#   of 5.7 and a cosine similarity of 0.83 are not comparable numbers. Averaging or
#   summing them would let whichever system happens to produce larger raw values
#   dominate the merge. Rank is scale-invariant: 1st place is 1st place regardless
#   of the underlying score. RRF converts each ranking to 1/(position+1) weights,
#   so a document ranked #1 by BM25 and #2 by semantic gets a fair, additive boost
#   from both signals without any calibration step.
