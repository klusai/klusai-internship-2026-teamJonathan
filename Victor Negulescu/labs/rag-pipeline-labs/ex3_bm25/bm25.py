...
class BM25Index:
	...
	def score(self, query_terms: list[str], doc_idx: int) -> float:
		...
		for term in query_terms:
			f = doc_toks.count(term)
			if f == 0:
				continue
			numerator = f * (K1 + 1)
			denominator = f + K1 * (1 - B + B * dl / avg)
...
# --- Recorded results (python bm25.py) ---
# BM25 recall@k over the 7 clean queries:
#   recall@1: 5/7 = 71%
#   recall@2: 6/7 = 86%
#   recall@3: 6/7 = 86%
#
# Rare-term spotlight — Q3 "What is Project Halibut?":
#   "Halibut" is the rank-1 chunk: True (score=5.72)
#
# Query BM25 handles poorly — Q4 "keep employees from leaving":
#   The query uses paraphrase language ("keep from leaving") that shares no tokens
#   with the gold section header "People and Culture" or its discussion of attrition.
#   BM25 returns the executive-summary section at rank 1 because it matches common
#   words from the query, not the section that actually answers it. Semantic search
#   retrieves the correct section immediately because embeddings capture meaning.
