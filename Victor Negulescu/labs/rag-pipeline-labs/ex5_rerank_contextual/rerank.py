...
# word overlap.
RERANK_SYSTEM = (
	"You are a document reranker. Given a query and a set of candidate documents, "
	"reorder the documents from most to least relevant by calling rank_documents.\n\n"
	"Judge relevance by how well each document actually answers the query's intent — "
	"not by surface word overlap or shared keywords. A document that addresses the "
	"query's underlying question is more relevant than one that merely repeats its words."
)
...
# --- Recorded results (model=claude-sonnet-4-6) ---
# gold at rank 1 — before: 0/3   after rerank: 3/3
# The reranker correctly promoted the gold section to rank 1 in all three cases,
# including Q8 ("what changed after the phishing attack") which requires understanding
# that the query is about incident response, not software engineering or executive summaries.
