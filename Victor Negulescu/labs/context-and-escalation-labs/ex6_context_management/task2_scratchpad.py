...
def synthesize(client: anthropic.Anthropic, scratchpad_path: Path) -> str:
	...
	resp = client.messages.create(
		model=MODEL,
		max_tokens=256,
		system="You are an incident triage assistant writing an end-of-shift summary.",
		messages=[
			{
				"role": "user",
				"content": (
					"Below is the triage scratchpad for this shift. "
					"Write a short summary: total incidents by severity (P1/P2/P3) "
					"and list which incident IDs are P1.\n\n"
					f"{scratchpad}"
				),
			}
		],
...
# per-step tokens (scratchpad): [150, 116, 89, 116, 96, 118, 107, 99, 112, 83, 114, 83]  — flat
# per-step tokens (naive):      [150, 246, 315, 411, 487, 585, 672, 751, 843, 906, 1000, 1063]  — climbing
# total input tokens scratchpad path: 1283
# total input tokens naive path:      7429  (5.8x more)
# final synthesis reads (scratchpad):  282 tokens
#   vs. re-reading all 12 raw records: 1051 tokens
#
# Severities matched gold (P1={1,6,9}, P2={2,4,7,11}, P3={3,5,8,10,12}): all correct.
